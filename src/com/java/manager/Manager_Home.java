...
public class Manager_Home extends JFrame{
	...
	public DefaultTableModel sales_tm;
	...
	public JScrollPane T3_sales;
	public JScrollPane T4_stock;
	public JTable stock_t;
	public JTable sales_t;
	...
	public void Tab1_p(JPanel T1_panel, String id) {
	    ...
	    board_t.getColumnModel().getColumn(5).setCellRenderer(new Tab1_TableCell(this, login_id, title));
	    board_t.getColumnModel().getColumn(5).setCellEditor(new Tab1_TableCell(this, login_id, title));
	...
	private void Tab2_p(JPanel T2_panel) {
		...
		JButton insert_p = new JButton("제품추가");
		insert_p.setBounds(10, 70, 100, 40);
		T2_panel.add(insert_p);
	    ...
	    product_t.getColumnModel().getColumn(6).setCellRenderer(new Tab2_TableCell(this));
	    product_t.getColumnModel().getColumn(6).setCellEditor(new Tab2_TableCell(this));
	...
	// 매출관리
	private void Tab3_p(JPanel T3_panel) {

		
		Font font = new Font("맑은고딕", Font.BOLD, 25);
		Font font1 = new Font("맑은고딕", Font.BOLD, 13);

		JLabel title_lb = new JLabel("매출통계");
		title_lb.setBounds(10, 5, 100, 30);
		title_lb.setFont(font);
		...
		String sales[] = { "지점명", "날짜", "총매출"};
		sales_tm = new DefaultTableModel();
		sales_tm.setColumnIdentifiers(sales);
		sales_t = new JTable(sales_tm);
		
		dtos = dao.sales_select();
		
		for(int i  = 0; i<dtos.size(); i++) {
			sales_tm.addRow(new Object[]{dtos.get(i).getPlace(), dtos.get(i).getSale_date(),dtos.get(i).getSale_total()});
		}
		sales_t.setRowHeight(30);
		sales_t.getTableHeader().setReorderingAllowed(false); // 컬럼들 이동 불가
		sales_t.getTableHeader().setResizingAllowed(false); // 컬럼 크기 조절 불가
		
		T3_sales = new JScrollPane(sales_t);
		T3_sales.setBounds(0, 120, 687, 280);
		T3_panel.add(T3_sales);
		
		selectbtn.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				sales_tm.setNumRows(0);
				...
				sales_t.setRowSelectionInterval(0, 0);
			}
		});
	...
	// 매장재고관리
	private void Tab4_p(JPanel T4_panel) {
		Font font = new Font("맑은고딕", Font.BOLD, 25);
		...
		JButton insertbtn = new JButton("재고추가");
		insertbtn.setBounds(5, 70, 100, 40);
		T4_panel.add(insertbtn);

		JTextField search = new JTextField();
		search.setBounds(230, 5, 300, 40);
		search.setText("매장명");
		T4_panel.add(search);

		JButton searchbtn = new JButton("검색");
		searchbtn.setBounds(550, 5, 100, 40);
		T4_panel.add(searchbtn);

		JButton select_p = new JButton("전체조회");
		select_p.setBounds(550, 70, 100, 40);
		T4_panel.add(select_p);
		...
		stock_t.getColumnModel().getColumn(8).setCellRenderer(new Tab3_TableCell(this));
	    stock_t.getColumnModel().getColumn(8).setCellEditor(new Tab3_TableCell(this));
		...
		select_p.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				stock_t.setRowSelectionInterval(0, 0);
			}
		});
		...
		searchbtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				dtos = dao.stock_search(p_search);
